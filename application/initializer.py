...
class IncludeAPIRouter(object):
    def __new__(cls):
        ...
        from application.main.routers.index import router as router_index
        ...
        from application.main.routers.image_classifier import (
            router as router_image_classification,
        )
        ...
        router.include_router(router_index, prefix="", tags=["index"])
        router.include_router(
            router_image_classification, prefix="/api/v1", tags=["image_classification"]
        )
...
# load model first to make it easy when using model again
tf.keras.models.load_model(settings.APP_CONFIG.DR_CLASSIFICATION_MODEL)
